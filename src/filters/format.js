...
export const isFormartDate = (phptime, fmt = 'yyyy-MM-dd hh:mm:ss') => {
 ...
 */
export const subString = (content, lng, flag = false) => {
  if (!content) {
    return ''
  }
  lng = lng || 20
 ...
 * @param {int} time 时间戳
 */
export function timeAgo (time, t = 'zh') {
  ...
  if (between < 3600) {
    if (between < 60) {
      return String(t) === 'zh' ? '刚刚' : 'a moment ago'
    }
